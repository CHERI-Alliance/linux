...
{
	unsigned long addr = __c_pa(kasan_reset_tag(x));
		...
		phys_addr_t phys_addr, pgprot_t prot)
{
	...
	if (addr != __c_pa(area->addr) ||
	    __c_fakep(end) != area->addr + get_vm_area_size(area)) {
		WARN_ONCE(1, "ioremap request [%lx,%lx) doesn't match vm_area [%lx, %lx)\n",
			  addr, end, __c_pa(area->addr),
			  __c_pa(area->addr) + get_vm_area_size(area));
		return -ERANGE;
	}
...
{
	vm->flags = flags;
	vm->addr = cheri_make_kernel_data_cap(va->va_start,
...
static void purge_vmap_node(struct work_struct *work)
{
	...
	list_for_each_entry_safe(va, n_va, &vn->purge_list, list) {
		...
		if (is_vmalloc_or_module_addr(__c_fakep(orig_start)))
...
void vm_unmap_ram(const void *mem, unsigned int count)
{
	...
	debug_check_no_locks_freed(__c_fakep(va->va_start),
				    (va->va_end - va->va_start));
...
void __init vm_area_register_early(struct vm_struct *vm, size_t align)
{
	...
	for (p = &vmlist; (cur = *p) != NULL; p = &cur->next) {
		if (__c_pa(cur->addr) - addr >= vm->size)
			break;
...
struct vm_struct *remove_vm_area(const void *addr)
{
	...
	if (WARN(!PAGE_ALIGNED(__c_pa(addr)), "Trying to vfree() bad address (%p)\n",
			addr))
		return NULL;
...
long vread_iter(struct iov_iter *iter, const char *addr, size_t count)
{
	...
	if (__c_pa(addr) + remains <= va->va_start)
		goto finished_zero;
				...
				unsigned long size)
{
	...
	if (!PAGE_ALIGNED(uaddr) || !PAGE_ALIGNED(__c_pa(kaddr)))
		return -EINVAL;
	...
	for (i = 0; i < nr_vmap_nodes; i++) {
		...
		list_for_each_entry(va, &vn->lazy.head, list) {
			seq_printf(m, "0x%pK-0x%pK %7ld unpurged vm_area\n",
				__c_fakep(va->va_start), __c_fakep(va->va_end),
				va->va_end - va->va_start);
		}
		spin_unlock(&vn->lazy.lock);
	}
...
static void __init vmap_init_free_space(void)
{
	...
	for (busy = vmlist; busy; busy = busy->next) {
		if (__c_pa(busy->addr) - vmap_start > 0) {
