				      ...
				      struct snd_xferi __user *_xferi)
{
	...
	if (put_user(0, &_xferi->result))
		return -EFAULT;
	if (copy_from_user_with_ptr(&xferi, _xferi, sizeof(xferi)))
		return -EFAULT;
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
				      ...
				      struct snd_xfern __user *_xfern)
{
	...
	if (put_user(0, &_xfern->result))
		return -EFAULT;
	if (copy_from_user_with_ptr(&xfern, _xfern, sizeof(xfern)))
		return -EFAULT;
				 ...
				 unsigned int cmd, void __user *arg)
{
	...
	case SNDRV_PCM_IOCTL_LINK:
		return snd_pcm_link(substream, (int)(user_uintptr_t) arg);
	...
	case SNDRV_PCM_IOCTL_PAUSE:
		return snd_pcm_pause_lock_irq(substream, (user_uintptr_t)arg);
