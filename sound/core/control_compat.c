				    ...
				    struct snd_ctl_elem_list32 __user *data32)
{
	...
	if (copy_from_user_no_ptr(&data, data32, 4 * sizeof(u32)))
		return -EFAULT;
	/* pids */
	...
	if (err < 0)
		return err;
	/* copy the result */
	if (copy_to_user_no_ptr(data32, &data, 4 * sizeof(u32)))
		return -EFAULT;
	return 0;
}
				    ...
				    struct snd_ctl_elem_info32 __user *data32)
{
	...
	case SNDRV_CTL_ELEM_TYPE_ENUMERATED:
		if (copy_to_user_no_ptr(&data32->value.enumerated,
				   ...
				   int replace)
{
	...
	case SNDRV_CTL_ELEM_TYPE_INTEGER64:
		...
		break;
	case SNDRV_CTL_ELEM_TYPE_ENUMERATED:
		if (copy_from_user_no_ptr(&data->value.enumerated,
...
static inline long snd_ctl_ioctl_compat(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SNDRV_CTL_IOCTL_TLV_COMMAND:
		return snd_ctl_ioctl(file, cmd, (user_uintptr_t)argp);
