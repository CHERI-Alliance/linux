 ...
 *
 * FIXCHERI:
 * The PTE.CW bit will likely become mandatory for CHERI enabled
 * systems (regardless of PTE.CRG) but as long as we need to support
 * systems that need PTE.CW an others that don't allow us to set it
 * we try to probe:
 * - For sv57, sv49 and sv39 first set up page tables that
 *   don't have PTE.CW set.
 * - During the actual probing also try to write a capability
 *   and if this succeeds assume that PTE.CW is not supported.
 * - If it faults (for any reason) assume that PTE.CW is
 *   neccessary and try the probe again.
 * CAVEATS:
...
retry:
	...
	__pi_create_pgd_mapping(early_pg_dir,
			   set_satp_mode_pmd,
			   pgtable_l5_enabled ?  __c_pa(early_p4d) :
				(pgtable_l4_enabled ?  __c_pa(early_pud) :
				__c_pa(early_pmd)),
