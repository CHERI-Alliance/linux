				...
				uintptr_t val)
{
	...
	if (offset)
		*(register_t *)((uintptr_t)regs + offset) = val;
}
					...
					struct pt_regs *regs)
{
	...
	regs_set_gpr(regs, reg_err * sizeof(register_t), -EFAULT);
	regs_set_gpr(regs, reg_zero * sizeof(register_t), 0);
				  ...
				  struct pt_regs *regs)
{
	...
	addr = regs_get_gpr(regs, reg_addr * sizeof(register_t));
	...
	regs_set_gpr(regs, reg_data * sizeof(register_t), __c_fakeu(data));
