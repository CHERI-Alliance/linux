...
#define print_sqe(m, sqe, sq_idx, sq_shift, opcode)						\
	do {										\
...
static void __io_uring_show_fdinfo(struct io_ring_ctx *ctx, struct seq_file *m)
{
	...
	for (i = 0; i < sq_entries; i++) {
		...
		if (io_in_compat64(ctx))
			opcode = READ_ONCE(ctx->sq_sqes_compat[sq_off].opcode);
		else
			opcode = READ_ONCE(ctx->sq_sqes[sq_off].opcode);
		...
		if (io_in_compat64(ctx))
			print_sqe(m, &ctx->sq_sqes_compat[sq_off], sq_idx, sqe128, opcode);
		else
			print_sqe(m, &ctx->sq_sqes[sq_off], sq_idx, sqe128, opcode);
