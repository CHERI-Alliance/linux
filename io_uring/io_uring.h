...
{
	return IS_ENABLED(CONFIG_COMPAT64) && (ctx->int_flags & IO_RING_F_COMPAT);
}
...
#define BUILD_BUG_COMPAT_SQE_UNION_ELEM(elem1, elem2) \
	...
	if (sqe->opcode == IORING_OP_URING_CMD || sqe->opcode == IORING_OP_URING_CMD128) {
		...
		if (ctx) {
			if ((ctx->flags & IORING_SETUP_SQE128) ||
                            ((ctx->flags & IORING_SETUP_SQE_MIXED) && sqe->opcode == IORING_OP_URING_CMD128)) {
				native_cmd_size += sizeof(struct io_uring_sqe);
				compat_cmd_size += sizeof(struct __c64_io_uring_sqe);
			}
		}
