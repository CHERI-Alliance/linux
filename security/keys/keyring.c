...
{
	return __c_pa(x) & KEYRING_PTR_SUBTYPE;
}
...
static void hash_key_type_and_desc(struct keyring_index_key *index_key)
{
	...
	type = __c_pa(index_key->type);
	acc = mult_64x32_and_fold(type, desc_len + 13);
	acc = mult_64x32_and_fold(acc, 9207);
	piece = __c_pa(index_key->domain_tag);
...
static unsigned long keyring_get_key_chunk(const void *data, int level)
{
	...
	case 1:
		return index_key->x;
	case 2:
		return __c_pa(index_key->type);
	case 3:
		return __c_pa(index_key->domain_tag);
	default:
		level -= 4;
...
static int keyring_diff_objects(const void *object, const void *data)
{
	...
	seg_a = __c_pa(a->domain_tag);
	seg_b = __c_pa(b->domain_tag);
	if ((seg_a ^ seg_b) != 0)
		goto differ;
	level += sizeof(unsigned long);
