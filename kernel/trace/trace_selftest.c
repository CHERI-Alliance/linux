					...
					struct trace_array *tr)
{
	 ...
	 */
	ftrace_set_filter_ip(&direct, __c_pa(DYN_FTRACE_TEST_NAME), 0, 0);
	ret = register_ftrace_direct(&direct, (uintptr_t)ftrace_stub_direct_tramp);
	if (ret)
		goto out;
	...
	ret = unregister_ftrace_direct(&direct, __c_pa(ftrace_stub_direct_tramp),
				       true);
	if (ret)
		goto out;
