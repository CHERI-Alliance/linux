			 ...
			 bool lock_direct_mutex)
{
	...
	if (tr->func.ftrace_managed) {
		if (lock_direct_mutex)
			ret = modify_ftrace_direct(tr->fops, (uintptr_t)new_addr);
		else
			ret = modify_ftrace_direct_nolock(tr->fops, (uintptr_t)new_addr);
	} else {
...
static int register_fentry(struct bpf_trampoline *tr, void *new_addr)
{
	...
	if (tr->func.ftrace_managed) {
		...
		ret = register_ftrace_direct(tr->fops, (uintptr_t)new_addr);
	} else {
