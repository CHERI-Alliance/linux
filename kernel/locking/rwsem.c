...
# define DEBUG_RWSEMS_WARN_ON(c, sem)	do {			\
	if (!debug_locks_silent &&				\
		...
		__c_pa(sem->magic),				\
		__c_ua(atomic_ptr_read(&(sem)->owner)), __c_pa(current),	\
...
static inline void rwsem_set_owner(struct rw_semaphore *sem)
{
	...
	atomic_ptr_set(&sem->owner, (uintptr_t)current);
}
...
static inline void rwsem_clear_reader_owned(struct rw_semaphore *sem)
{
	...
	while ((val & ~RWSEM_OWNER_FLAGS_MASK) == (uintptr_t)current) {
...
{
	return (struct task_struct *)(uintptr_t)
