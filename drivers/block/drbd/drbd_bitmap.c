...
{
	/* use cmpxchg? */
	clear_bit(BM_PAGE_NEED_WRITEOUT, (unsigned long *)&page_private(page));
	clear_bit(BM_PAGE_LAZY_WRITEOUT, (unsigned long *)&page_private(page));
}

static void bm_set_page_need_writeout(struct page *page)
{
	set_bit(BM_PAGE_NEED_WRITEOUT, (unsigned long *)&page_private(page));
}
...
void drbd_bm_mark_for_writeout(struct drbd_device *device, int page_nr)
{
	...
	if (!test_and_set_bit(BM_PAGE_HINT_WRITEOUT, (unsigned long *)&page_private(page)))
		b->al_bitmap_hints[b->n_bitmap_hints++] = page_nr;
}

static int bm_test_page_unchanged(struct page *page)
{
	volatile const unsigned long *addr = (unsigned long *)&page_private(page);
	return (*addr & ((1UL<<BM_PAGE_NEED_WRITEOUT)|(1UL<<BM_PAGE_LAZY_WRITEOUT))) == 0;
}

static void bm_set_page_io_err(struct page *page)
{
	set_bit(BM_PAGE_IO_ERROR, (unsigned long *)&page_private(page));
}

static void bm_clear_page_io_err(struct page *page)
{
	clear_bit(BM_PAGE_IO_ERROR, (unsigned long *)&page_private(page));
}

static void bm_set_page_lazy_writeout(struct page *page)
{
	set_bit(BM_PAGE_LAZY_WRITEOUT, (unsigned long *)&page_private(page));
}

static int bm_test_page_lazy_writeout(struct page *page)
{
	return test_bit(BM_PAGE_LAZY_WRITEOUT, (unsigned long *)&page_private(page));
}
	...
	if (flags & BM_AIO_READ) {
		...
		for (hint = 0; hint < b->n_bitmap_hints; hint++) {
			...
			if (!test_and_clear_bit(BM_PAGE_HINT_WRITEOUT,
			    (unsigned long *)&page_private(b->bm_pages[i])))
				continue;
			/* Has it even changed? */
