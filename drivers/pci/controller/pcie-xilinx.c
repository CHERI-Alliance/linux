...
/* Top chip for the new decode mechanism */
static struct irq_chip xilinx_msi_decode_top_chip = {
	.name		= "PCIe MSI",
	.irq_ack	= irq_chip_ack_parent,
	.irq_mask       = xilinx_msi_decode_top_irq_mask,
	.irq_unmask     = xilinx_msi_decode_top_irq_unmask,
};

/* Deprecated FIFO based mechanism */
static struct irq_chip xilinx_msi_fifo_top_chip = {
	.name		= "PCIe MSI",
	.irq_ack	= xilinx_msi_fifo_top_irq_ack,
};
...
/* Again, two structs for the two different modes */
static struct msi_domain_info xilinx_msi_decode_info = {
	.flags	= MSI_FLAG_USE_DEF_DOM_OPS | MSI_FLAG_USE_DEF_CHIP_OPS |
		  MSI_FLAG_NO_AFFINITY,
	.chip	= &xilinx_msi_decode_top_chip,
};

static struct msi_domain_info xilinx_msi_fifo_info = {
	.flags	= MSI_FLAG_USE_DEF_DOM_OPS | MSI_FLAG_USE_DEF_CHIP_OPS |
		  MSI_FLAG_NO_AFFINITY,
	.chip	= &xilinx_msi_fifo_top_chip,
};
...
static int xilinx_allocate_msi_domains(struct xilinx_pcie *pcie)
{
	...
	msi_info = pcie->msi_decode ? &xilinx_msi_decode_info
				    : &xilinx_msi_fifo_info;

	pcie->msi_domain = pci_msi_create_irq_domain(fwnode, msi_info, parent);
	if (!pcie->msi_domain) {
