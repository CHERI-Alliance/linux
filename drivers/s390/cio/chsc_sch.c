...
out_free_chsc:
	free_page((uintptr_t)on_close_chsc_area);
...
out_free:
	kfree(cd);
	free_page((uintptr_t)scpcd_area);
	return ret;
}
...
out_free:
	kfree(cd);
	free_page((uintptr_t)scucd_area);
	return ret;
}
...
out_free:
	kfree(cud);
	free_page((uintptr_t)sscud_area);
	return ret;
}
...
out_free:
	kfree(dcal);
	free_page((uintptr_t)sdcal_area);
	return ret;
}
