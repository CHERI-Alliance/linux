...
{
	enum max77857_id id = __c_pa(dev_get_drvdata(dev));
...
{
	enum max77857_id id = __c_pa(rdev_get_drvdata(rdev));
	unsigned int regval;
	int ret;
...
static int max77857_probe(struct i2c_client *client)
{
	...
	id = __c_ua(i2c_id->driver_data);

	dev_set_drvdata(dev, __c_fakep(id));
