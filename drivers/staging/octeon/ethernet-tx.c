...
static void cvm_oct_tx_do_cleanup(uintptr_t arg);
...
netdev_tx_t cvm_oct_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (skb_shinfo(skb)->nr_frags == 0) {
		hw_buffer.s.addr = XKPHYS_TO_PHYS(__c_pa(skb->data));
		hw_buffer.s.pool = 0;
		...
		for (i = 0; i < skb_shinfo(skb)->nr_frags; i++) {
			...
			hw_buffer.s.addr =
				XKPHYS_TO_PHYS(__c_pa(skb_frag_address(fs)));
			hw_buffer.s.size = skb_frag_size(fs);
			CVM_OCT_SKB_CB(skb)[i + 1] = hw_buffer.u64;
		}
		hw_buffer.s.addr =
			XKPHYS_TO_PHYS(__c_pa(CVM_OCT_SKB_CB(skb)));
