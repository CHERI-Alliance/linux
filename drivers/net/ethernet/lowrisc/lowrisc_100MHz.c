...
static inline void eth_write(struct net_local *priv, size_t addr, int data)
{
	volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
	eth_base[addr >> 3] = data;
}

static inline void eth_copyout(struct net_local *priv, uint8_t *data, int len)
{
	int i, rnd = ((len - 1) | 7) + 1;
	volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
	if (!(((size_t)data) & 7)) {
...
static inline int eth_read(struct net_local *priv, size_t addr)
{
	volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
	return eth_base[addr >> 3];
}

static inline void eth_copyin(struct net_local *priv, uint8_t *data, int len,
			      int start)
{
	int i, rnd = ((len - 1) | 7) + 1;
	volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
...
static inline void eth_enable_irq(struct net_local *priv)
{
	volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
...
static inline void eth_disable_irq(struct net_local *priv)
{
	volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
	eth_base[MACHI_OFFSET >> 3] &= ~MACHI_IRQ_EN;
	mmiowb();
...
static void lowrisc_update_address(struct net_local *priv,
...
static int lowrisc_mii_probe(struct net_device *ndev)
{
	...
	phy_connect_direct(ndev, phydev, lowrisc_phy_adjust_link, PHY_INTERFACE_MODE_MII);
...
{
	struct net_local *priv = (struct net_local *)
		ctrl; /* struct mdiobb_ctrl must be first in net_local for bitbang driver to work */
	volatile uint64_t *eth_base = (volatile uint64_t *)(priv->ioaddr);
	if (dir)
		priv->last_mdio_gpio |= MDIOCTRL_MDIOOEN_MASK; // output driving
	else
		priv->last_mdio_gpio &=
			~MDIOCTRL_MDIOOEN_MASK; // input receiving
...
static int lowrisc_mii_init(struct net_device *ndev)
{
	...
	if (mdiobus_register(priv->mii_bus)) {
		netif_warn(priv, probe, priv->ndev,
...
static int lowrisc_ether_poll(struct napi_struct *napiptr, int budget)
{
	...
	while ((rsr & RSR_RECV_DONE_MASK) && (rx_count < budget)) {
		int len = eth_read(priv, RPLR_OFFSET + ((buf & 7) << 3)) -
			  4; /* discard FCS bytes ?? */
		if ((len >= 60) && (len <= ETH_FRAME_LEN + ETH_FCS_LEN)) {
			int rnd = ((len - 1) | 7) +
				  1; /* round to a multiple of 8 */
			struct sk_buff *skb = napi_alloc_skb(
				&priv->napi, rnd); // drop surplus packets
			if (unlikely(!skb)) {
				/* Couldn't get memory, we carry on regardless and drop if necessary */
				ndev->stats.rx_dropped++;
			} else {
				int start =
					RXBUFF_OFFSET / 8 + ((buf & 7) << 8);
				skb_put(skb,
					len); /* Tell the skb how much data we got */
...
static void lowrisc_get_regs(struct net_device *ndev, struct ethtool_regs *regs,
			     void *p)
{
	struct net_local *priv = netdev_priv(ndev);
	...
	for (i = 0; i < LOWRISC_REGS_LEN; i++) {
		if (i >= 32)
			regs_buff[i] =
				eth_read(priv, MACLO_OFFSET + ((i - 32) << 3));
		else {
...
static int lowrisc_open(struct net_device *ndev)
{
	...
	retval = lowrisc_mii_init(ndev);
	if (retval) {
		dev_err(&ndev->dev, "Failed to initialize Phy\n");
		retval = -ENODEV;
		return retval;
	}
 ...
 * Return:	0, always.
 */
static netdev_tx_t lowrisc_send(struct sk_buff *new_skb,
				struct net_device *ndev)
{
	struct net_local *priv = netdev_priv(ndev);
...
static int lowrisc_mii_ioctl(struct net_device *netdev, struct ifreq *ifr,
			     int cmd)
{
...
static int lowrisc_100MHz_probe(struct platform_device *ofdev)
{
	...
	priv->ndev = ndev;
	priv->ioaddr = devm_platform_get_and_ioremap_resource(ofdev, 0, &lowrisc_ethernet);
	mutex_init(&priv->lock);

	ndev->netdev_ops = &lowrisc_netdev_ops;
	ndev->flags &= ~IFF_MULTICAST;
	ndev->watchdog_timeo = TX_TIMEOUT;
	netif_napi_add_weight(ndev, &priv->napi, lowrisc_ether_poll, 64);

	printk("lowrisc-digilent-ethernet: Lowrisc ethernet platform (%llX-%llX) mapped to %lx\n",
	       lowrisc_ethernet[0].start, lowrisc_ethernet[0].end,
	       (size_t)(priv->ioaddr));
...
static void lowrisc_100MHz_free(struct platform_device *of_dev)
{
	struct resource *iomem =
		platform_get_resource(of_dev, IORESOURCE_MEM, 0);
	release_mem_region(iomem->start, resource_size(iomem));
}

static void lowrisc_100MHz_unregister(struct platform_device *of_dev)
{
