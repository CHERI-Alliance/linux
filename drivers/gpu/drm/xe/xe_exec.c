...
int xe_exec_ioctl(struct drm_device *dev, void *data, struct drm_file *file)
{
	...
	/*
	 * FIXCHERI: Check the type of thing that is in addresses and check
	 * if we should check capabilities or actually leave the ABI at u64.
	 */
	if (xe_exec_queue_is_parallel(q)) {
		err = copy_from_user_with_ptr(addresses, addresses_user, sizeof(u64) *
					      q->width);
		if (err) {
			err = -EFAULT;
			goto err_syncs;
