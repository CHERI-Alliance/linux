                                    ...
                                    int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_START:			
		...
		memset32((void *)((uintptr_t)chip->pvReg + SRAM_OUTPUT_BASE), 0, SRAM_OUTPUT_SIZE/4);
		break;
									  ...
									  int cmd)
 {
	...
	case SNDRV_PCM_TRIGGER_START:		
		...
		memset32((void *)((uintptr_t)chip->pvReg + SRAM_INPUT_BASE), 0, SRAM_INPUT_SIZE/4);
		break;
	case SNDRV_PCM_TRIGGER_STOP:
...
static int snd_smi_play_copy_data(struct sm768chip *chip,int sramTxSection)
{
	...
	if(play_substream == NULL)
		memset32((void *)((uintptr_t)chip->pvReg + SRAM_OUTPUT_BASE) + SRAM_SECTION_SIZE * sramTxSection, 0x00, P_PERIOD_BYTE/4);
	else{
...
static int snd_smi_capture_copy_data(struct sm768chip *chip,int sramTxSection)
{
	...
	if(capture_substream == NULL)	
		memset32((void *)((uintptr_t)chip->pvReg + SRAM_INPUT_BASE + SRAM_SECTION_SIZE * sramTxSection), 0x00,  P_PERIOD_BYTE/4);
                                         ...
                                         struct sm768chip **smichip)
{
	...
	*smichip = NULL;
	...
	//clear SRAM
	memset32((void *)((uintptr_t)chip->pvReg + SRAM_OUTPUT_BASE), 0, SRAM_TOTAL_SIZE/4);
