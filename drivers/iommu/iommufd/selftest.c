				    ...
				    size_t length, void __user *uptr)
{
	...
	if (iova % MOCK_IO_PAGE_SIZE || length % MOCK_IO_PAGE_SIZE ||
	    ...
	    check_add_overflow(__c_pa(uptr), length, &end))
		return -EINVAL;
	...
	for (; length; length -= MOCK_IO_PAGE_SIZE) {
		...
		npages = get_user_pages_fast(__c_pa(uptr) & PAGE_MASK, 1, 0,
					     pages);
		if (npages < 0) {
			rc = npages;
			goto out_put;
				      ...
				      unsigned int refs)
{
	...
	for (; length; length -= PAGE_SIZE) {
		...
		npages = get_user_pages_fast(__c_pa(uptr), 1, 0, pages);
		if (npages < 0)
			return npages;
		if (WARN_ON(npages != 1))
			return -EFAULT;
				    ...
				    size_t npages)
{
	for (; npages; npages--) {
		...
		rc = get_user_pages_fast(__c_pa(uptr), 1, 0, tmp_pages);
		if (rc < 0)
			return rc;
		if (WARN_ON(rc != 1))
			return -EFAULT;
