		     ...
		     void __user *userptr)
{
	...
	for (off = 0, num = 0; off < total_size; off += e->next_offset, num++){
		...
		e = loc_cpu_entry + off;
		if (copy_to_user_with_ptr(userptr + off, e,
				 offsetof(struct ip6t_entry, counters)) ||
		    copy_to_user_with_ptr(userptr + off
				 + offsetof(struct ip6t_entry, counters),
				 &counters[num],
				 sizeof(counters[num]))) {
			ret = -EFAULT;
			  ...
			  unsigned int i)
{
	...
	origsize = *size;
	ce = *dstptr;
	if (copy_to_user_with_ptr(ce, e, offsetof(struct compat_ip6t_entry, counters)) ||
	    copy_to_user_with_ptr(&ce->counters, &counters[i], sizeof(counters[i])))
		return -EFAULT;
