...
TRACE_EVENT(io_uring_queue_async_work,
	...
	TP_printk("ring " TRACE_CAP_FMT ", request " TRACE_CAP_FMT ", user_data 0x%llx, opcode %s, flags 0x%llx, %s queue, work " TRACE_CAP_FMT "",
		...
		__entry->rw ? "hashed" : "normal", __get_cap(work))
);
...
TP_PROTO(struct io_ring_ctx *ctx, void *req, struct io_uring_cqe *cqe),
	...
	TP_fast_assign(
		...
		__entry->extra1		= ctx->flags & IORING_SETUP_CQE32 || cqe->flags & IORING_CQE_F_32 ? __c_ua(cqe->big_cqe[0]) : 0;
		__entry->extra2		= ctx->flags & IORING_SETUP_CQE32 || cqe->flags & IORING_CQE_F_32 ? __c_ua(cqe->big_cqe[1]) : 0;
	),
