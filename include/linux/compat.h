...
__c64c_ptr_array_from_user(bool iscompat, void *dst, void __user *src,
			   unsigned long cnt)
{
	unsigned long ret;
	unsigned int i;
	...
	if (likely(!iscompat))
		return copy_from_user_with_ptr(dst, src,
					       cnt * sizeof(user_uintptr_t));

	ret = copy_from_user_no_ptr(dst, src, cnt * sizeof(__c64_uptr));
	if (ret)
		return ret;
	for (i = cnt; i--;)
		dp[i] = (user_uintptr_t)compat_ptr(sp[i]);

	return 0;
}
#define __c64_ptr_array_from_user(dst, src, cnt)			\
	__c64c_ptr_array_from_user(in_compat64_syscall(), dst, src, cnt)
