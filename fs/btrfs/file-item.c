...
found:
	...
	read_extent_buffer(path->nodes[0], dst, __c_pa(item),
			ret * csum_size);
out:
...
found:
	...
	ins_size *= csum_size;
	ins_size = min_t(u32, __c_pa(item_end) - __c_pa(item), ins_size);
	write_extent_buffer(leaf, sums->sums + index, __c_pa(item), ins_size);
