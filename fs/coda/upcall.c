...
{ 
	...
	if (!error) {
		...
		if (retlen >= *length)
			retlen = *length - 1;
		*length = retlen;
		result =  (char *)outp + (long)__c_pa(outp->coda_readlink.data);
		 ...
		 unsigned int cmd, struct PioctlData *data)
{
	...
	if (copy_from_user((char *)inp + (long)__c_pa(inp->coda_ioctl.data),
	...
	if (outsize < (long)__c_pa(outp->coda_ioctl.data) + outp->coda_ioctl.len) {
		error = -EINVAL;
		goto exit;
	}
	...
	if (copy_to_user(data->vi.out,
			 (char *)outp + (long)__c_pa(outp->coda_ioctl.data),
			 outp->coda_ioctl.len)) {
		error = -EFAULT;
		goto exit;
	}
