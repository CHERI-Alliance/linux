			       ...
			       bool is_upper)
{
	...
	inode = ilookup5(sb, __c_pa(key), ovl_inode_test, key);
	if (!inode)
		return NULL;
...
bool ovl_lookup_trap_inode(struct super_block *sb, struct dentry *dir)
{
	...
	trap = ilookup5(sb, __c_pa(key), ovl_inode_test, key);
	if (!trap)
		return false;
...
struct inode *ovl_get_trap_inode(struct super_block *sb, struct dentry *dir)
{
	...
	trap = iget5_locked(sb, __c_pa(key), ovl_inode_test,
			    ovl_inode_set, key);
	if (!trap)
		return ERR_PTR(-ENOMEM);
...
static struct inode *ovl_iget5(struct super_block *sb, struct inode *newinode,
			       struct inode *key)
{
	return newinode ? inode_insert5(newinode, __c_pa(key),
					 ovl_inode_test, ovl_inode_set, key) :
			  iget5_locked(sb, __c_pa(key),
				       ovl_inode_test, ovl_inode_set, key);
}
